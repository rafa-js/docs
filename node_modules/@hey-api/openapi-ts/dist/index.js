...
`,a));});};var Oe=async({context:e$1})=>{let n=x.resolve(e$1.config.output.path);e$1.config.output.clean&&g(n);let o=e(d(e$1.config.output.tsConfigPath)),r=o?.options.moduleResolution===kt.ModuleResolutionKind.NodeNext,t=m(e$1.config);"bundle"in t.config&&t.config.bundle&&y({outputPath:n,plugin:t,tsConfig:o});for(let s of e$1.registerPlugins())await s.run();if(!e$1.config.dryRun){let s=e$1.createFile({id:"_index",path:"index"});for(let a of Object.values(e$1.files))if(a.nameWithoutExtension()!==s.nameWithoutExtension()){if(!a.isEmpty()&&a.exportFromIndex&&e$1.config.output.indexFile){let u$1=s.relativePathToFile({context:e$1,id:a.id});r&&(u$1.startsWith("./")||u$1.startsWith("../"))&&(u$1==="./client"?u$1="./client/index.js":u$1=`${u$1}.js`),s.add(u.exportAllDeclaration({module:u$1}));}a.write(`

`,o);}s.isEmpty()||s.write(`
`,o);for(let a of e$1.gen.render({moduleResolution:o?.options.moduleResolution})){if(!a.content)continue;let i=x.resolve(n,a.path),u=x.dirname(i);R.mkdirSync(u,{recursive:true}),R.writeFileSync(i,a.content,{encoding:"utf8"});}}};var He=({patchOptions:e,spec:n})=>{if(!e)return;let o=n;if("swagger"in o){if(e.version&&o.swagger&&(o.swagger=typeof e.version=="string"?e.version:e.version(o.swagger)),e.meta&&o.info&&e.meta(o.info),e.schemas&&o.definitions)for(let r in e.schemas){let t=o.definitions[r];if(!t||typeof t!="object")continue;let s=e.schemas[r];s(t);}if(e.operations&&o.paths)for(let r in e.operations){let[t,s]=r.split(" ");if(!t||!s)continue;let a=o.paths[s];if(!a)continue;let i=a[t.toLocaleLowerCase()]||a[t.toLocaleUpperCase()];if(!i||typeof i!="object")continue;let u=e.operations[r];u(i);}return}if(e.version&&o.openapi&&(o.openapi=typeof e.version=="string"?e.version:e.version(o.openapi)),e.meta&&o.info&&e.meta(o.info),o.components){if(e.schemas&&o.components.schemas)for(let r in e.schemas){let t=o.components.schemas[r];if(!t||typeof t!="object")continue;let s=e.schemas[r];s(t);}if(e.parameters&&o.components.parameters)for(let r in e.parameters){let t=o.components.parameters[r];if(!t||typeof t!="object")continue;let s=e.parameters[r];s(t);}if(e.requestBodies&&o.components.requestBodies)for(let r in e.requestBodies){let t=o.components.requestBodies[r];if(!t||typeof t!="object")continue;let s=e.requestBodies[r];s(t);}if(e.responses&&o.components.responses)for(let r in e.responses){let t=o.components.responses[r];if(!t||typeof t!="object")continue;let s=e.responses[r];s(t);}}if(e.operations&&o.paths)for(let r in e.operations){let[t,s]=r.split(" ");if(!t||!s)continue;let a=o.paths[s];if(!a)continue;let i=a[t.toLocaleLowerCase()]||a[t.toLocaleUpperCase()];if(!i||typeof i!="object")continue;let u=e.operations[r];u(i);}};var oe=c(Pn(),1),nr={biome:{args:e=>["format","--write",e],command:"biome",name:"Biome (Format)"},prettier:{args:e=>["--ignore-unknown",e,"--write","--ignore-path","./.prettierignore"],command:"prettier",name:"Prettier"}},tr={biome:{args:e=>["lint","--apply",e],command:"biome",name:"Biome (Lint)"},eslint:{args:e=>[e,"--fix"],command:"eslint",name:"ESLint"},oxlint:{args:e=>["--fix",e],command:"oxlint",name:"oxlint"}},Cn=({config:e})=>{if(e.output.format){let n=nr[e.output.format];console.log(`\u2728 Running ${n.name}`),(0, oe.sync)(n.command,n.args(e.output.path));}if(e.output.lint){let n=tr[e.output.lint];console.log(`\u2728 Running ${n.name}`),(0, oe.sync)(n.command,n.args(e.output.path));}};function bn(e,n){return {...e,config:n,models:e.models.map(o=>rr(o)),services:or(e.operations).map(sr),types:{}}}var rr=e=>({...e,$refs:e.$refs.filter((n,o,r)=>B$1(n,o,r)),enum:e.enum.filter((n,o,r)=>r.findIndex(t=>t.value===n.value)===o),enums:e.enums.filter((n,o,r)=>r.findIndex(t=>t.name===n.name)===o),imports:e.imports.filter((n,o,r)=>B$1(n,o,r)&&n!==e.name).sort(C)}),or=e=>{let n$1=n(),o=new Map;return e.forEach(r=>{(r.tags?.length&&(n$1.plugins["@hey-api/sdk"]?.config.asClass||p$1(n$1))?r.tags.filter(B$1):["Default"]).forEach(s=>{let a={...r,service:lr(s)},i=o.get(a.service)||ar(a);i.$refs=[...i.$refs,...a.$refs],i.imports=[...i.imports,...a.imports],i.operations=[...i.operations,a],o.set(a.service,i);});}),Array.from(o.values())},sr=e=>{let n={...e};return n.operations=ir(n),n.operations.forEach(o=>{n.imports.push(...o.imports);}),n.imports=n.imports.filter(B$1).sort(C),n},ir=e=>{let n=new Map;return e.operations.map(o=>{let r={...o};r.imports.push(...r.parameters.flatMap(i=>i.imports));let t=r.responses.filter(i=>i.responseTypes.includes("success"));r.imports.push(...t.flatMap(i=>i.imports));let s=r.name,a=n.get(s)||0;return a>0&&(r.name=`${s}${a}`),n.set(s,a+1),r})},ar=e=>({$refs:[],imports:[],name:e.service,operations:[]}),lr=e=>w({case:"PascalCase",value:z(e)});var se=e=>e.startsWith(q),pr=e=>{let n={path:""};if(e.path&&(typeof e.path!="string"||!se(e.path)))return n.path=e.path,n;let[o,r]=e.path.split("?"),s=(r||"").split("&").map(P=>P.split("=")),a=o||"";a.endsWith("/")&&(a=a.slice(0,a.length-1));let[,i]=a.split("://"),[u,l,c]=(i||"").split("/");n.organization=l||e.organization,n.project=c||e.project;let f=[],m="api_key";n.api_key=s.find(([P])=>P===m)?.[1]||e.api_key||process.env.HEY_API_TOKEN,n.api_key&&f.push(`${m}=${n.api_key}`);let d="branch";n.branch=s.find(([P])=>P===d)?.[1]||e.branch,n.branch&&f.push(`${d}=${n.branch}`);let y="commit_sha";n.commit_sha=s.find(([P])=>P===y)?.[1]||e.commit_sha,n.commit_sha&&f.push(`${y}=${n.commit_sha}`);let q="tags";n.tags=s.find(([P])=>P===q)?.[1]?.split(",")||e.tags,n.tags?.length&&f.push(`${q}=${n.tags.join(",")}`);let k="version";if(n.version=s.find(([P])=>P===k)?.[1]||e.version,n.version&&f.push(`${k}=${n.version}`),!n.organization)throw new Error("missing organization - from which Hey API Platform organization do you want to generate your output?");if(!n.project)throw new Error("missing project - from which Hey API Platform project do you want to generate your output?");let T=f.join("&"),S=u||"get.heyapi.dev",ue=S.startsWith("localhost"),pe=[ue?"http":"https",S].join("://"),ce=ue?[pe,"v1","get",n.organization,n.project].join("/"):[pe,n.organization,n.project].join("/");return n.path=T?`${ce}?${T}`:ce,n},qn=e=>{let n=b$1.cyan("Generating from");if(typeof e.path=="string"){let o=se(e.path)?`${e.organization??""}/${e.project??""}`:e.path;console.log(`\u23F3 ${n} ${o}`),se(e.path)&&(e.branch&&console.log(`${b$1.gray("branch:")} ${b$1.green(e.branch)}`),e.commit_sha&&console.log(`${b$1.gray("commit:")} ${b$1.green(e.commit_sha)}`),e.tags?.length&&console.log(`${b$1.gray("tags:")} ${b$1.green(e.tags.join(", "))}`),e.version&&console.log(`${b$1.gray("version:")} ${b$1.green(e.version)}`));}else console.log(`\u23F3 ${n} raw OpenAPI specification`);},ie=async({config:e,dependencies:n,logger:o$1,templates:r,watch:t})=>{let s=pr(e.input),{timeout:a}=e.input.watch,i=t||{headers:new Headers};e.logs.level!=="silent"&&!t&&qn(s);let u=o$1.timeEvent("spec"),{data:l,error:c,response:f}=await L({fetchOptions:e.input.fetch,inputPath:s.path,timeout:a,watch:i});if(u.timeEnd(),c&&!t)throw new Error(`Request failed with status ${f.status}: ${f.statusText}`);let m,d;if(l){e.logs.level!=="silent"&&t&&(console.clear(),qn(s));let y=o$1.timeEvent("input.patch");He({patchOptions:e.parser.patch,spec:l}),y.timeEnd();let q=o$1.timeEvent("parser");if(e.experimentalParser&&!o(e)&&!p$1(e)&&(d=F({config:e,dependencies:n,logger:o$1,spec:l})),!d){let S=E({openApi:l});m=bn(S,e);}q.timeEnd();let k=o$1.timeEvent("generator");d?await Oe({context:d}):m&&await we({client:m,openApi:l,templates:r}),k.timeEnd();let T=o$1.timeEvent("postprocess");if(!e.dryRun&&(Cn({config:e}),e.logs.level!=="silent")){let S=process.env.INIT_CWD?`./${x.relative(process.env.INIT_CWD,e.output.path)}`:e.output.path;console.log(`${b$1.green("\u{1F680} Done!")} Your output is in ${b$1.cyanBright(S)}`);}T.timeEnd();}return e.input.watch.enabled&&typeof s.path=="string"&&setTimeout(()=>{ie({config:e,dependencies:n,logger:o$1,templates:r,watch:i});},e.input.watch.interval),d||m};var An={1:function(e,n,o,r,t){return `import { NgModule} from '@angular/core';
...
`+((s=i(o,"equals").call(a,i(i(i(i(t,"root"),"$config"),"client"),"name"),"legacy/angular",{name:"equals",hash:{},fn:e.program(8,t,0),inverse:e.program(11,t,0),data:t,loc:{start:{line:23,column:0},end:{line:87,column:11}}}))!=null?s:"")},useData:true};var Tn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getHeaders = <T>(config: OpenAPIConfig, options: ApiRequestOptions<T>): Observable<HttpHeaders> => {
	return forkJoin({
		// @ts-ignore
...
};`},useData:true};var En={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getRequestBody = (options: ApiRequestOptions): unknown => {
	if (options.body) {
	...
	return undefined;
};`},useData:true};var kn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getResponseBody = <T>(response: HttpResponse<T>): T | undefined => {
	if (response.status !== 204 && response.body !== null) {
		return response.body;
	}
	return undefined;
};`},useData:true};var wn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getResponseHeader = <T>(response: HttpResponse<T>, responseHeader?: string): string | undefined => {
	if (responseHeader) {
	...
	return undefined;
};`},useData:true};var Sn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){var s,a=e.lookupProperty||function(i,u){if(Object.prototype.hasOwnProperty.call(i,u))return i[u]};return `import { HttpClient, HttpHeaders } from '@angular/common/http';
...
};`},useData:true};var Hn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `import type { ApiRequestOptions } from './ApiRequestOptions';
import type { ApiResult } from './ApiResult';
...
export class ApiError extends Error {
...
}`},useData:true};var Dn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export type ApiRequestOptions<T = unknown> = {
	readonly body?: any;
	...
	readonly url: string;
};`},useData:true};var In={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export type ApiResult<TData = any> = {
	readonly body: TData;
	readonly ok: boolean;
	readonly status: number;
	readonly statusText: string;
	readonly url: string;
};`},useData:true};var Bn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getHeaders = async <T>(config: OpenAPIConfig, options: ApiRequestOptions<T>): Promise<Record<string, string>> => {
	...
	return headers;
};`},useData:true};var jn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getRequestBody = (options: ApiRequestOptions): unknown => {
	if (options.body) {
		return options.body;
	}
	return undefined;
};`},useData:true};var $n={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getResponseBody = (response: AxiosResponse<unknown>): unknown => {
	if (response.status !== 204) {
		return response.data;
	}
	return undefined;
};`},useData:true};var Nn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getResponseHeader = (response: AxiosResponse<unknown>, responseHeader?: string): string | undefined => {
	if (responseHeader) {
	...
	return undefined;
};`},useData:true};var Fn={1:function(e,n,o,r,t){return "ApiResult<T>"},3:function(e,n,o,r,t){return "T"},5:function(e,n,o,r,t){return "result.body"},7:function(e,n,o,r,t){return "result"},compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){var s,a=n??(e.nullContext||{}),i=e.lookupProperty||function(u,l){if(Object.prototype.hasOwnProperty.call(u,l))return u[l]};return `import axios from 'axios';
...
};`},usePartial:true,useData:true};var _n={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const sendRequest = async <T>(
	config: OpenAPIConfig,
	options: ApiRequestOptions<T>,
	url: string,
	body: unknown,
...
};`},useData:true};var Ln={1:function(e,n,o,r,t){return `import type { HttpClient } from '@angular/common/http';
import type { Observable } from 'rxjs';
...
`+((s=i(o,"equals").call(a,i(i(i(i(t,"root"),"$config"),"client"),"name"),"legacy/angular",{name:"equals",hash:{},fn:e.program(9,t,0),inverse:e.program(11,t,0),data:t,loc:{start:{line:24,column:1},end:{line:28,column:12}}}))!=null?s:"")+"}"},useData:true};var Wn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export class CancelError extends Error {
...
export class CancelablePromise<T> implements Promise<T> {
...
}`},useData:true};var Mn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getHeaders = async <T>(config: OpenAPIConfig, options: ApiRequestOptions<T>): Promise<Headers> => {
	const [token, username, password, additionalHeaders] = await Promise.all([
		// @ts-ignore
		resolve(options, config.TOKEN),
		// @ts-ignore
...
};`},useData:true};var Un={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getRequestBody = (options: ApiRequestOptions): unknown => {
	if (options.body !== undefined) {
		if (options.mediaType?.includes('application/json') || options.mediaType?.includes('+json')) {
			return JSON.stringify(options.body);
		} else if (isString(options.body) || isBlob(options.body) || isFormData(options.body)) {
	...
	return undefined;
};`},useData:true};var Vn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getResponseBody = async (response: Response): Promise<unknown> => {
	if (response.status !== 204) {
		try {
	...
	return undefined;
};`},useData:true};var Gn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getResponseHeader = (response: Response, responseHeader?: string): string | undefined => {
	if (responseHeader) {
	...
	return undefined;
};`},useData:true};var zn={1:function(e,n,o,r,t){return `import fetch, { FormData, Headers } from 'node-fetch';
...
};`},usePartial:true,useData:true};var Kn={1:function(e,n,o,r,t){return `	if (config.WITH_CREDENTIALS) {
...
`+((s=a(o,"equals").call(n??(e.nullContext||{}),a(a(a(a(t,"root"),"$config"),"client"),"name"),"legacy/fetch",{name:"equals",hash:{},fn:e.program(1,t,0),inverse:e.noop,data:t,loc:{start:{line:19,column:1},end:{line:23,column:12}}}))!=null?s:"")+`
...
};`},useData:true};var Xn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const base64 = (str: string): string => {
	try {
		return btoa(str);
	} catch (err) {
		// @ts-ignore
...
};`},useData:true};var Qn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const catchErrorCodes = (options: ApiRequestOptions, result: ApiResult): void => {
...
};`},useData:true};var Jn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getFormData = (options: ApiRequestOptions): FormData | undefined => {
	if (options.formData) {
	...
	return undefined;
};`},useData:true};var Yn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getQueryString = (params: Record<string, unknown>): string => {
	const qs: string[] = [];
...
};`},useData:true};var Zn={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `const getUrl = (config: OpenAPIConfig, options: ApiRequestOptions): string => {
...
};`},useData:true};var et={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const isBlob = (value: any): value is Blob => {
	return value instanceof Blob;
};`},useData:true};var nt={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const isFormData = (value: unknown): value is FormData => {
	return value instanceof FormData;
};`},useData:true};var tt={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const isString = (value: unknown): value is string => {
	return typeof value === 'string';
};`},useData:true};var rt={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const isStringWithValue = (value: unknown): value is string => {
	return isString(value) && value !== '';
};`},useData:true};var ot={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const isSuccess = (status: number): boolean => {
	return status >= 200 && status < 300;
};`},useData:true};var st={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `type Resolver<T> = (options: ApiRequestOptions<T>) => Promise<T>;
...
export const resolve = async <T>(options: ApiRequestOptions<T>, resolver?: T | Resolver<T>): Promise<T | undefined> => {
	...
	return resolver;
};`},useData:true};var it={1:function(e,n,o,r,t){return `import { Inject, Injectable } from '@angular/core';
...
`+((s=i(o,"equals").call(a,i(i(i(i(t,"root"),"$config"),"client"),"name"),"legacy/angular",{name:"equals",hash:{},fn:e.program(11,t,0),inverse:e.program(13,t,0),data:t,loc:{start:{line:38,column:1},end:{line:58,column:12}}}))!=null?s:"")+"}"},useData:true};var at={1:function(e,n,o,r,t){return `import type { HttpResponse } from '@angular/common/http';
...
`+((s=l(o,"notEquals").call(a,l(l(l(l(t,"root"),"$config"),"client"),"name"),"legacy/angular",{name:"notEquals",hash:{},fn:e.program(15,t,0),inverse:e.noop,data:t,loc:{start:{line:79,column:2},end:{line:81,column:16}}}))!=null?s:"")+`		response: new Interceptors(),
	...
	return new Headers(headers);
};`},useData:true};var pt={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getRequestBody = (options: ApiRequestOptions): unknown => {
	if (options.body !== undefined) {
		if (options.mediaType?.includes('application/json') || options.mediaType?.includes('+json')) {
			return JSON.stringify(options.body);
		} else if (isString(options.body) || isBlob(options.body) || isFormData(options.body)) {
	...
	return undefined;
};`},useData:true};var ct={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getResponseBody = (xhr: XMLHttpRequest): unknown => {
	if (xhr.status !== 204) {
		try {
	...
	return undefined;
};`},useData:true};var ft={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const getResponseHeader = (xhr: XMLHttpRequest, responseHeader?: string): string | undefined => {
	if (responseHeader) {
	...
	return undefined;
};`},useData:true};var mt={1:function(e,n,o,r,t){return "ApiResult<T>"},3:function(e,n,o,r,t){return "T"},5:function(e,n,o,r,t){return "result.body"},7:function(e,n,o,r,t){return "result"},compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){var s,a=n??(e.nullContext||{}),i=e.lookupProperty||function(u,l){if(Object.prototype.hasOwnProperty.call(u,l))return u[l]};return `import { ApiError } from './ApiError';
import type { ApiRequestOptions } from './ApiRequestOptions';
import type { ApiResult } from './ApiResult';
import { CancelablePromise } from './CancelablePromise';
import type { OnCancel } from './CancelablePromise';
...
};`},usePartial:true,useData:true};var dt={compiler:[8,">= 4.3.0"],main:function(e,n,o,r,t){return `export const sendRequest = async (
	config: OpenAPIConfig,
	options: ApiRequestOptions,
	url: string,
	body: any,
...
};`},useData:true};var cr=()=>{p.registerHelper("camelCase",function(e){return w({case:"camelCase",value:e})}),p.registerHelper("equals",function(e,n,o){return e===n?o.fn(this):o.inverse(this)}),p.registerHelper("ifServicesResponse",function(e,n$1){return n().plugins["@hey-api/sdk"]?.config.response===e?n$1.fn(this):n$1.inverse(this)}),p.registerHelper("ifdef",function(...e){let n=e.pop();return e.every(o=>!o)?n.inverse(this):n.fn(this)}),p.registerHelper("notEquals",function(e,n,o){return e!==n?o.fn(this):o.inverse(this)}),p.registerHelper("transformServiceName",function(e){return A({config:n(),name:e})});},gt=()=>{cr();let e={client:p.template(An),core:{apiError:p.template(Hn),apiRequestOptions:p.template(Dn),apiResult:p.template(In),baseHttpRequest:p.template(Ln),cancelablePromise:p.template(Wn),httpRequest:p.template(it),request:p.template(lt),settings:p.template(at)}};return p.registerPartial("functions/base64",p.template(Xn)),p.registerPartial("functions/catchErrorCodes",p.template(Qn)),p.registerPartial("functions/getFormData",p.template(Jn)),p.registerPartial("functions/getQueryString",p.template(Yn)),p.registerPartial("functions/getUrl",p.template(Zn)),p.registerPartial("functions/isBlob",p.template(et)),p.registerPartial("functions/isFormData",p.template(nt)),p.registerPartial("functions/isString",p.template(tt)),p.registerPartial("functions/isStringWithValue",p.template(rt)),p.registerPartial("functions/isSuccess",p.template(ot)),p.registerPartial("functions/resolve",p.template(st)),p.registerPartial("fetch/getHeaders",p.template(Mn)),p.registerPartial("fetch/getRequestBody",p.template(Un)),p.registerPartial("fetch/getResponseBody",p.template(Vn)),p.registerPartial("fetch/getResponseHeader",p.template(Gn)),p.registerPartial("fetch/request",p.template(zn)),p.registerPartial("fetch/sendRequest",p.template(Kn)),p.registerPartial("xhr/getHeaders",p.template(ut)),p.registerPartial("xhr/getRequestBody",p.template(pt)),p.registerPartial("xhr/getResponseBody",p.template(ct)),p.registerPartial("xhr/getResponseHeader",p.template(ft)),p.registerPartial("xhr/request",p.template(mt)),p.registerPartial("xhr/sendRequest",p.template(dt)),p.registerPartial("axios/getHeaders",p.template(Bn)),p.registerPartial("axios/getRequestBody",p.template(jn)),p.registerPartial("axios/getResponseBody",p.template($n)),p.registerPartial("axios/getResponseHeader",p.template(Nn)),p.registerPartial("axios/request",p.template(Fn)),p.registerPartial("axios/sendRequest",p.template(_n)),p.registerPartial("angular/getHeaders",p.template(Tn)),p.registerPartial("angular/getRequestBody",p.template(En)),p.registerPartial("angular/getResponseBody",p.template(kn)),p.registerPartial("angular/getResponseHeader",p.template(wn)),p.registerPartial("angular/request",p.template(Sn)),p.registerPartial("angular/sendRequest",p.template(On)),e};var fr=0,yt=e=>`${e}-${fr++}`,ae=e=>`${e}-end`,ht=e=>`${e}-length`,le=e=>`${e}-start`,mr=(e,n)=>{if(e>200)return {color:b$1.red,type:"duration"};if(n>30)return {color:b$1.red,type:"percentage"};if(e>50)return {color:b$1.yellow,type:"duration"};if(n>10)return {color:b$1.yellow,type:"percentage"}},B=class{events=[];end(n){let o,r=this.events;for(let t of n.position)o=r[t],o?.events&&(r=o.events);o&&!o.end&&(o.end=performance.mark(ae(o.id)));}report(n=true){let o=this.events[0];if(!o)return;let r=this.events[this.events.length-1],t="root",s=yt(t),a=performance.measure(ht(s),le(o.id),ae(r.id));return n&&this.reportEvent({end:r.end,events:this.events,id:s,indent:0,measure:a,name:t,start:o.start}),a}reportEvent({indent:n,...o}){let r=n?b$1.gray:b$1.cyan,t=o.events.length-1;o.events.forEach((s,a)=>{let i=performance.measure(ht(s.id),le(s.id),ae(s.id)),u=Math.ceil(i.duration*100)/100,l=Math.ceil(i.duration/o.measure.duration*100*100)/100,c=n?mr(u,l):void 0,f=`${u.toFixed(2).padStart(8)}ms`;c?.type==="duration"&&(f=c.color(f));let m=a===t?"\u2514\u2500 ":"\u251C\u2500 ",d=n?"\u2502  ".repeat(n-1)+m:"",y=38-d.length,q=n?"\u21B3 ":"",T=`${n?" ".repeat(n-1)+q:""}${l.toFixed(2)}%`;c?.type==="percentage"&&(T=c.color(T)),console.log(b$1.gray(d)+r(`${s.name.padEnd(y)} ${f} (${T})`)),this.reportEvent({...s,indent:n+1,measure:i});});}start(n){return performance.mark(le(n))}storeEvent({result:n,...o}){let r=o.events.length-1,t=o.events[r];if(t&&!t.end){n.position=[...n.position,r],this.storeEvent({...o,events:t.events,result:n});return}let s=o.events.push({...o,events:[]});n.position=[...n.position,s-1];}timeEvent(n){let o=yt(n),r=this.start(o),t={events:this.events,id:o,name:n,start:r},s={position:[]};return this.storeEvent({...t,result:s}),{mark:r,timeEnd:()=>this.end(s)}}};var dr={stringCase:w};b$1.enabled=yr().hasBasic;var Ni=async(e,n=new B)=>{let o=typeof e=="function"?await e():e,r$1=[];try{Ae();let t=n.timeEvent("createClient"),s=n.timeEvent("config"),a=await K(o);for(let m of a.results)if(r$1.push(m.config),m.errors.length)throw m.errors[0];s.timeEnd();let i=n.timeEvent("handlebars"),u=gt();i.timeEnd();let c=(await Promise.all(r$1.map(m=>ie({config:m,dependencies:a.dependencies,logger:n,templates:u})))).filter(m=>!!m);t.timeEnd();let f=r$1[0];return n.report(f&&f.logs.level==="debug"),c}catch(t){let s=r$1[0],a=s?s.dryRun:o?.dryRun,i$1=s?s.interactive:o?.interactive,u=s?.logs??r(o),l$1;throw u.level!=="silent"&&u.file&&!a&&(l$1=i(t,u.path??"")),u.level!=="silent"&&(k({error:t,logPath:l$1}),await l({error:t,isInteractive:i$1})&&await j(t)),t}},Fi=async e=>typeof e=="function"?await e():e;export{B as Logger,Ni as createClient,Fi as defineConfig,dr as utils};//# sourceMappingURL=index.js.map
